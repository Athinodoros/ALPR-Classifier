...
int main(int argv, char** argc) {
	...
	VideoCapture cap("VID_20180222_131327.mp4"); // open the default camera
...
void findPossiblePlatesClassifier(Mat& in, Mat& out) {
	...
	// Detect plates
	std::vector<Rect> platesdetected;
	face_cascade.detectMultiScale(src, platesdetected, 1.2, 1, 0 | CASCADE_SCALE_IMAGE);
	Mat newRound;
	// Draw circles on the detected faces
	Rect biggest;
	...
	for (int i = 0; i < platesdetected.size(); i++)
	{
		Size deltaSize(platesdetected[i].width * 0.1f, platesdetected[i].height * 0.1f); // 0.1f = 10/100
		...
		ellipse(src, center, Size(platesdetected[i].width*0.5, platesdetected[i].height*0.5), 0, 0, 360, Scalar(255, 0, 255), 4, 8, 0);
